...
const generateLevelData = (level: number): LevelData => {
  ...
  const solution: LevelSolution = { landmark1: questionLandmarks[0], landmark2: questionLandmarks[1], distance, orientation, line, start, end };
...
const CustomMapScalesChallenge2: React.FC<LevelComponentProps> = ({ onComplete, onExit, partialProgress, onSavePartialProgress }) => {
  ...
  useEffect(() => {
    if (!levelData || levelData.level !== currentLevel) {
      ...
      if (levelData && levelData.level !== currentLevel) {
        setPlacedBlocks([]); setIsMeasurementComplete(false); setIsLevelComplete(false);
        setEquationAnswer(''); setScaleAnswer(''); setCountAnswer(''); setIsEquationScaleHintActive(false);
      }
    }
  ...
  useEffect(() => {
    if (!levelData) return;
    if (placedBlocks.length === levelData.solution.distance) setIsMeasurementComplete(true);
  }, [placedBlocks, levelData]);

  useEffect(() => {
    return () => { if (!isCompletedRef.current && onSavePartialProgress) onSavePartialProgress({ currentLevel, levelData, placedBlocks, isMeasurementComplete, equationAnswer, scaleAnswer, countAnswer, inputErrorCount }); };
  ...
  const handleInvalidDrop = (reason: 'scale' | 'location') => {
    ...
    if (reason === 'scale') { setInvalidDropScaleHintActive(true); setTimeout(() => setInvalidDropScaleHintActive(false), 2500); }
  };
  ...
  const handleSubmitEquation = useCallback((e: React.FormEvent) => {
    ...
    if (currentLevel >= 4) { if (countAnswer !== correctCount) { setCountError(true); allCorrect = false; setTimeout(() => { setCountError(false); setCountAnswer(''); }, 500); } }
    if (currentLevel >= 2) { if (scaleAnswer !== correctScale) { setScaleError(true); allCorrect = false; setTimeout(() => { setScaleError(false); setScaleAnswer(''); }, 500); } }
    if (equationAnswer !== correctTotal) { setEquationError(true); allCorrect = false; setTimeout(() => { setEquationError(false); setEquationAnswer(''); }, 500); }
    if (allCorrect) {
      setIsLevelComplete(true);
      if (currentLevel === TOTAL_LEVELS) {
        const rating = (inputErrorCount <= 2) ? 3 : (inputErrorCount <= 5) ? 2 : 1;
        isCompletedRef.current = true; onComplete(rating); setFinalStars(rating);
      } else {
        // AUTO-ADVANCE: Proceed without modal
        setTimeout(() => setCurrentLevel(prev => prev + 1), 2000);
      }
    } else { setInputErrorCount(prev => prev + 1); }
  }, [levelData, currentLevel, countAnswer, scaleAnswer, equationAnswer, inputErrorCount, onComplete]);

  const handleReplay = () => {
    setFinalStars(null); setInputErrorCount(0); isCompletedRef.current = false; onSavePartialProgress?.(null);
    setPlacedBlocks([]); setIsMeasurementComplete(false); setIsLevelComplete(false); setEquationAnswer(''); setScaleAnswer(''); setCountAnswer(''); setCurrentLevel(1);
  };
  ...
  return (
    <DndProvider backend={backendForDND} options={{ enableMouseEvents: true }}>
      <main className="bg-slate-100 min-h-screen w-full flex items-center justify-center p-4 font-sans relative">
        ...
        <InstructionModal isOpen={isInstructionModalOpen} onClose={() => setIsInstructionModalOpen(false)} title="Map Scales Practice Instructions">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-7xl relative">
          ...
          <div className="grid lg:grid-cols-[240px_1fr_240px] gap-x-6 items-start mt-2">
            <aside>
              ...
              <BlockPalette level={currentLevel} scale={levelData.scale} baseUnitSize={cellPixelSize} showHint={paletteHintActive} onInteraction={() => setPaletteHintActive(false)} />
            </aside>

            <div className="flex flex-col items-center relative" ref={gridWrapperRef}>
              <GridMap level={currentLevel} rows={GRID_ROWS} cols={GRID_COLS} landmarks={levelData.landmarks} scale={levelData.scale} solution={levelData.solution} placedBlocks={placedBlocks} isCompleted={isLevelComplete} onAddBlock={handleAddBlock} onInvalidDrop={handleInvalidDrop} checkPlacement={checkPlacement} scaleHintActive={invalidDropScaleHintActive || isEquationScaleHintActive} />
              <Equation level={currentLevel} count={levelData.solution.distance} scale={levelData.scale} correctTotal={levelData.solution.distance * levelData.scale} isMeasurementComplete={isMeasurementComplete} isLevelComplete={isLevelComplete} onAnswerSubmit={handleSubmitEquation} answer={equationAnswer} setAnswer={setEquationAnswer} equationError={equationError} scaleAnswer={scaleAnswer} setScaleAnswer={setScaleAnswer} scaleError={scaleError} countAnswer={countAnswer} setCountAnswer={setCountAnswer} countError={countError} isScaleHintVisible={isEquationScaleHintActive} onScaleHintToggle={setIsEquationScaleHintActive} />

              {/* SUCCESS FEEDBACK OVERLAY (NON-MODAL) */}
              {isLevelComplete && currentLevel < TOTAL_LEVELS && (
        ...
        {finalStars !== null && (
          <div className="fixed inset-0 bg-black bg-opacity-80 flex flex-col justify-center items-center z-[100] p-4 antialiased animate-fade-in">
            <div className="bg-white p-6 md:p-10 rounded-xl shadow-2xl text-center max-w-md w-full">
              ...
              {finalStars < 3 && <p className="text-sm md:text-base text-gray-700/70 mt-3 md:mt-4 px-2">Solve with fewer errors to earn more stars.</p>}
